  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { 
      ...
      max-width: 650px; 
      margin: auto; 
      background: linear-gradient(135deg, #eef2f7 0%, #e5ebf4 100%); 
      color: var(--text);
      min-height: 100vh;
    }
    ...
    /* GIAO DIỆN HIỂN THỊ LINH VẬT TRÒN (CHO CẢ GIÁO VIÊN VÀ HỌC SINH) */
    .lobby-grid {
      display: grid;
...
<body>
  ...
  <div id="role-screen" class="card">
    ...
    <button onclick="selectRole('teacher')" style="background: linear-gradient(135deg, #4361ee 0%, #3a0ca3 100%); width: 100%; padding: 15px; font-size: 16px;">
    ...
    <button onclick="selectRole('admin')" style="background: #334155; width: 100%; margin-top: 12px; padding: 12px; font-size: 14px;">
  ...
  <div id="auth-screen" class="card" style="display: none;">
    ...
    <p style="text-align: center; color: var(--text-muted); font-size: 14px; margin-bottom: 20px;">Đăng nhập để quản lý kho tài liệu và đề thi riêng của thầy/cô</p>
  ...
  <div id="admin-screen" class="card" style="display: none;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
      <div>
        <h2 style="margin: 0; font-size: 20px; color: #334155;">⚙️ Quản Trị Hệ Thống (Admin)</h2>
  ...
  <div id="teacher-screen" class="card" style="display: none;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
      <div>
        ...
        <small style="color: var(--text-muted);">Tài liệu được bảo mật trên tài khoản này</small>
      </div>
    ...
    <!-- MÃ QR PHÒNG THI -->
    <div id="qrcode-box">
      <h3 style="margin: 0; color: var(--primary); font-size: 17px;">📲 QUÉT MÃ QR THAM GIA</h3>
      <p style="margin: 4px 0 0 0; font-size: 13px; color: var(--text-muted);">(Đoàn sinh mở Camera điện thoại quét mã để vào phòng)</p>
      <div id="qrcode"></div>
    </div>
    ...
    <div style="border-top: 2px dashed #cbd5e1; padding-top: 16px; margin-top: 16px;">
      ...
      <button id="start-btn" onclick="startQuiz()" style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); font-size: 18px; width: 100%; padding: 16px; box-shadow: 0 6px 15px rgba(239, 68, 68, 0.3);">
        🚀 BẮT ĐẦU LÀM BÀI
      </button>
    </div>

    <div id="teacher-live-box" style="display: none; margin-top: 20px; background: #eef2ff; padding: 16px; border-radius: 16px; border: 2px solid var(--primary); text-align: center;">
  ...
  <div id="student-screen" class="card" style="display: none; text-align: center;">
    ...
    <p style="color: var(--text-muted); font-size: 14px; margin-top: 0; margin-bottom: 20px;">Nhập tên của em để nhận Linh Vật may mắn</p>
  ...
  <div id="lobby-screen" class="card" style="display: none; text-align: center;">
    ...
    <div class="my-mascot-card">
      ...
      <p style="margin: 2px 0 0 0; font-size: 15px; opacity: 0.95;">Đoàn sinh: <strong id="lobby-my-name">--</strong></p>
    </div>

    <div style="background: #f8fafc; border: 2px dashed #cbd5e1; padding: 12px; border-radius: 16px;">
      <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 4px;">
        <span style="font-weight: 800; font-size: 13px; color: var(--primary);">👥 CÁC BẠN ĐÃ VÀO PHÒNG (<span id="lobby-count">0</span>)</span>
  ...
  <div id="result-screen" class="card" style="display: none;">
    ...
    <div id="teacher-result-view" style="display: none;">
      ...
      <button onclick="location.reload()" style="background: var(--primary); width: 100%; margin-top: 20px; padding: 14px;">Quay Lại Màn Hình Quản Trị</button>
    </div>
  ...
  <script>
    ...
    async function generateRoomQRCode() {
      const qrContainer = document.getElementById("qrcode");
      qrContainer.innerHTML = "";
      let hostUrl = window.location.origin;

      new QRCode(qrContainer, {
        text: `${hostUrl}?role=student`,
        width: 160,
        height: 160
      });
    }
    ...
    /* ĐOÀN SINH THAM GIA PHÒNG CHỜ LINH VẬT */
    function joinQuiz() {
      ...
      socket.emit('join_room', { name: name, role: 'student' });
    }
    ...
    function startQuiz() {
      ...
      socket.emit('start_quiz', { parts, quizName: selectedQuizName });
    }

    /* CẬP NHẬT LINH VẬT TRÒN CHO CẢ GIÁO VIÊN VÀ HỌC SINH */
    socket.on('update_students', (students) => {
      ...
      // Màn hình Giáo Viên hiển thị linh vật
      if (myRole === 'teacher') {
      ...
      // Màn hình Phòng Chờ Học Sinh hiển thị linh vật
      if (myRole === 'student') {
      ...
      if (myRole === 'teacher') {
        ...
        if (q.type === 'multiple') {
          q.options.forEach((opt, optIdx) => {
            if (opt) html += `<button class="opt-btn" type="button" onclick="selectChoice(${optIdx == q.correct}, this)">${opt}</button>`;
          });
        } else if (q.type === 'tf') {
          html += `<button class="opt-btn" type="button" onclick="selectChoice('${q.correct}' === 'Đúng', this)">Đúng</button>
                   <button class="opt-btn" type="button" onclick="selectChoice('${q.correct}' === 'Sai', this)">Sai</button>`;
        }
      ...
      localTimerInterval = setInterval(() => {
        ...
        if (currentRemainingSeconds <= 0) {
          ...
          if (waitingBox) {
            ...
            waitingBox.innerHTML = `⌛ <b>ĐÃ HẾT GIỜ!</b> Chuẩn bị câu tiếp theo sau <b>${restSeconds}s</b>...`;
          }
